...
/**
 * hwpx ZIP 패키징 (section0.xml + 선택적으로 header.xml 교체)
 */
async function packageHwpx(baseZipBuffer, sectionXml, headerXml = null) {
  const baseZip = await JSZip.loadAsync(baseZipBuffer);
  const newZip = new JSZip();

  // 기존 파일 복사 + section0.xml, header.xml 교체
  for (const [filename, file] of Object.entries(baseZip.files)) {
    ...
    } else if (filename === "Contents/header.xml" && headerXml) {
      newZip.file(filename, headerXml, { compression: "DEFLATE" });
    } else {
...
export async function createCombinedConsentHwpx(services, templates) {
  ...
  const usedTemplates = new Set([firstOrigFilename]);
  ...
  for (const svc of services.slice(1)) {
    ...
    usedTemplates.add(svcOrigFilename);
  ...
  // header.xml 병합: 다른 템플릿에만 있는 borderFill 추가
  let mergedHeader = null;
  if (usedTemplates.size > 1) {
    const baseHeader = await baseZip.file("Contents/header.xml").async("string");
    const baseFillIds = new Set(
      [...baseHeader.matchAll(/id="(\d+)"[^>]*>.*?<\/hh:borderFill>/gs)].map(m => m[1])
    );
    // 더 간단하게: base header의 borderFill id 목록
    const baseIds = new Set(
      [...baseHeader.matchAll(/<hh:borderFill\s[^>]*id="(\d+)"/g)].map(m => m[1])
    );

    let extraFills = "";
    for (const tplName of usedTemplates) {
      if (tplName === firstOrigFilename) continue;
      const tplBuffer = templates[tplName];
      const tplZip = await JSZip.loadAsync(tplBuffer);
      const tplHeader = await tplZip.file("Contents/header.xml").async("string");

      // 이 템플릿에만 있는 borderFill 찾기
      const fillRegex = /<hh:borderFill\s[^>]*id="(\d+)"[^>]*>.*?<\/hh:borderFill>/gs;
      let match;
      while ((match = fillRegex.exec(tplHeader)) !== null) {
        if (!baseIds.has(match[1])) {
          extraFills += match[0];
          baseIds.add(match[1]);
        }
      }
    ...
    if (extraFills) {
      // </hh:borderFills> 태그 앞에 추가
      mergedHeader = baseHeader.replace("</hh:borderFills>", extraFills + "</hh:borderFills>");
      // itemCnt 업데이트
      mergedHeader = mergedHeader.replace(
        /<hh:borderFills\s+itemCnt="(\d+)"/,
        (m, cnt) => `<hh:borderFills itemCnt="${baseIds.size}"`
      );
    }
  }

  return await packageHwpx(firstTemplate, finalSec, mergedHeader);
}
